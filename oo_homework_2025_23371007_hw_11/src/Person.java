...
public class Person implements PersonInterface {
    ...
    public List<MessageInterface> getReceivedMessages() {
        List<MessageInterface> result = new LinkedList<>();
        int length = Math.min(5, this.messages.size());
        for (int i = 0; i < length; i++) {
            result.add(this.messages.get(i));
        }
        return result;
    }
    ...
    public void deleteReceivedArticle(int articleId) {
        int index;
        while ((index = this.receivedArticles.indexOf(articleId)) != -1) {
            this.receivedArticles.remove(index);
        }
    }
