...
public class PathTest {//包含题目[3]
    ...
    public void testExtractLoopPath() {
        ...
        if (oldPath.getNodes().size() == oldPath.getDistinctNodes().size() || !oldPath.isValid()) {
            assertNull(oldPath.extractLoopPath());
        } else {
            assertTrue(oldPath.isValid());
            assertNotEquals(oldPath.getNodes().size(), oldPath.getDistinctNodes().size());

            Path loopPath = oldPath.extractLoopPath();
            assertNotNull(loopPath);
            assertTrue(loopPath.size() >= 2);
            assertTrue(loopPath.getNodes().size() == loopPath.getDistinctNodes().size() + 1);
            ...
            for (int i = 0; i < oldNodes.size() - 1; i++) {
                ...
                for (int j = 0; j < loopNodes.size(); j++) {
                    if (i + j >= oldNodes.size() && loopNodes.get(j) != oldNodes.get(i + j)) {
                        find = false;
                        break;
                    }
                }
